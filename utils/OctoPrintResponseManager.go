...
func GetOctoPrintResponseManagerInstance(client *octoprintApis.Client) (*octoPrintResponseManager) {
	if octoPrintResponseManagerInstance == nil {
		if client == nil {
			panic("GetOctoPrintResponseManagerInstance() was called for the first time, but the client passed was nil")
		}
...
func (this *octoPrintResponseManager) createBackgroundTask() {
	...
	duration := GetExperimentalFrequency(10, "EXPERIMENTAL_OCTO_PRINT_RESPONSE_MANGER_UPDATE_FREQUENCY")
	this.backgroundTask = CreateBackgroundTask(duration, this.update)
...
func (this *octoPrintResponseManager) update() {
	...
	connectionManager := GetConnectionManagerInstance(this.Client)
