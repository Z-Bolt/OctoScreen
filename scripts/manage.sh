...
while [[ "$SOURCE" != /dev/fd/* ]]; do DIR="$( cd -P "$( dirname "$SOURCE" )" >/dev/null 2>&1 && pwd )"; SOURCE="$(readlink "$SOURCE")"; [[ $SOURCE != /* ]] && SOURCE="$DIR/$SOURCE"; done;
...
optparse.command name="install" description="Install OctoScreen" dispatch="octoscreen.install"
optparse.command name="*" description="Unknown Command" dispatch="echo \"Unknown Command\"" help=hide

yes_no=( 'yes' 'no' )

function octoscreen.install() {
...
. <( optparse.build name=octoscreen description="OctoScreen Manager" allow=positional exec=optparse.usage)
