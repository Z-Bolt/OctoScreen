...
while [[ -L "$SOURCE" && "$SOURCE" != /dev/fd/* ]]; do DIR="$( cd -P "$( dirname "$SOURCE" )" >/dev/null 2>&1 && pwd )"; SOURCE="$(readlink "$SOURCE")"; [[ $SOURCE != /* ]] && SOURCE="$DIR/$SOURCE"; done; [[ "$SOURCE" != /dev/fd/* ]] && DIR="$( cd -P "$( dirname "$SOURCE" )" >/dev/null 2>&1 && pwd )" || DIR=''; function __clear_opts(){ set +o errexit; trap - $1; declare -g _ret="1"; }; function __trap_EXIT(){ __clear_opts EXIT; [ -v PS1 ] && return 1 || exit 1; }; function __trap_SIGINT(){ __clear_opts SIGINT; [ -v PS1 ] && return 1 || exit 1; };
